...
/*
 * class Tableroo { private JFrame frame; private JPanel[][] tablero;
 * 
 * public void inicializa(int dimensionX, int dimensionY) { frame = new
 * JFrame(); frame.setBounds(100, 100, 600, 600);
 * frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
 * frame.getContentPane().setLayout(new GridLayout(dimensionX, dimensionY));
 * tablero = new JPanel[dimensionX][dimensionY];
 * 
 * for (int i = 0; i < dimensionX; i++) { for (int j = 0; j < dimensionY; j++) {
 * JPanel casilla = new JPanel();
 * casilla.setBorder(BorderFactory.createLineBorder(Color.BLACK));
 * frame.getContentPane().add(casilla); tablero[i][j] = casilla; } }
 * 
